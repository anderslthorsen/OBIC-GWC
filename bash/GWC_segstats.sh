...
#for subject in `cat GWC/GWC_include20Dec2021.txt`; do
	...
	#for hemi in lh rh; do
		...
		# Run mri_segstats for GWC (using fsaverage aparc). This is currently not used as the number of vertices would be different across subjects and between thickness vs GWC due to the transfomration to fsavarage space.
		#mri_segstats --annot fsaverage ${hemi} aparc --i $SUBJECTS_DIR/$subject/surf/lh.nu.w-g.avg_fsaverage.10.mgh --sum $output_dir/test.gwc.lh.sum

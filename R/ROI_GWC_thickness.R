...
# This script compares the difference in cortical thickness in OCD before vs after adjusting for Regional GWC
...
for(region in 1:length(ROI)){
  ...
  lmer.test.GWC_covariate <- lmer(r ~ Group + Sex + Age + SurfaceHoles + GWC_covariate + (1 | Site), data = data, REML = FALSE) 

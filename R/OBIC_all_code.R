...
# Plot GWC values using ggseg
  ...
  # Set up dataframe to hold results which are plotted by ggseg
  results_emmeans <- tibble(
    region = rep(c(ggplot_ROIs_spaced), 4),
    mean = as.double(NA),
    hemi = c(rep("left", 34), rep("right", 34), rep("left", 34), rep("right", 34)),
    groups = c(rep("OCD", 68), rep("HC", 68))
  )
  
  library(emmeans)
  
  # Calculate estimated marginal means for GWC per ROI per group
  for(hemi in c("lh","rh")){ # Loop over hemispheres
    ...
    for(region in ggplot_ROIs){ # Loop over regions
      ...
      #### Approach below uses the estimated marginal means, but seems to yield the exact same means for every ROI
      data$r <- data[[paste0(hemi,"_GWC_",region)]]
      lmer.test <- lmer(r ~ Group + Sex + Age + SurfaceHoles + (1 | Site), data = data, REML = FALSE) 
      estimated_means_df <- emmeans(lmer.test, specs = "Group")
      
      estimated_means_actual <- summary(estimated_means_df)$emmean # Saves estimated means for HC as [1] and OCD as [2]
      
      # Save estimated mean for OCD
      results_emmeans[i,"mean"] <- estimated_means_actual[2]
      # Save estimated mean for HC
      results_emmeans[i+68,"mean"] <- estimated_means_actual[1]
      
      #print(i) # For debugging
      #print(paste0(hemi,"_GWC_",region))
      #print(estimated_means_actual[2])
      #print(estimated_means_actual[1])
        
      
        #### Alternative approach which uses the predicted values of the linear model which is calculated separately for OCD and HC 
        #data$r <- data[[paste0(hemi,"_GWC_",region)]]
        #lmer.test <- lmer(r ~ Sex + Age + SurfaceHoles + (1 | Site), data = subset(data, data$Group == 0), REML = FALSE) 
        #estimated_mean_HC <- predict(lmer.test)
        
        #lmer.test <- lmer(r ~ Sex + Age + SurfaceHoles + (1 | Site), data = subset(data, data$Group == 1), REML = FALSE) 
       # estimated_mean_OCD <- predict(lmer.test)
        
        # Save estimated mean for OCD
      #  results_emmeans[i,"mean"] <- mean(estimated_mean_OCD)
        # Save estimated mean for HC
      #  results_emmeans[i+68,"mean"] <- mean(estimated_mean_HC)
        
       # print(i) # For debugging
      #  print(paste0(hemi,"_GWC_",region))
       # print(estimated_mean_OCD[2])
       # print(estimated_mean_HC[1])
        
        #### A third approach which uses the predicted values of the linear model which is calculated combined for OCD and HC 
        #data$r <- data[[paste0(hemi,"_GWC_",region)]]
        #lmer.test <- lmer(r ~ Group + Sex + Age + SurfaceHoles + (1 | Site), data = data, REML = FALSE) 
        #estimated_mean <- predict(lmer.test)
        
        # Save estimated mean for OCD
        #results_emmeans[i,"mean"] <- mean(subset(estimated_mean, data$Group == 1))
        # Save estimated mean for HC
        #results_emmeans[i+68,"mean"] <- mean(subset(estimated_mean, data$Group == 0))
        
        #print(i) # For debugging
        #print(paste0(hemi,"_GWC_",region))
        #print(mean(subset(estimated_mean, data$Group == 1)))
        #print(mean(subset(estimated_mean, data$Group == 0)))
  ...
  for(hemi in c("lh","rh")){ # Loop over hemispheres
    ...
    if (hemi == "lh"){ 
      ...
      for(group in c(1,0)){ # Loop over groups
    
      if (group == 1){
        i=i+0 # Set to zero as OCD are first in the dataframe
      } else if (group == 0){
        i=i+34
      }
      
      data_temp <- subset(data, Group == group)
      
        for(region in ggplot_ROIs){ # Loop over regions
          
         i=i+1
          results[i,"mean"]<-mean(data_temp[[paste0(hemi,"_GWC_",region)]])
...
results %>%
  ...
  facet_wrap(~groups) +
  labs(fill="Mean GWC") +
  ggtitle("Regional GWC in OCD and HC") +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank())
